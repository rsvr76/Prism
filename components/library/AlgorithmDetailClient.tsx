      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 md:p-8 space-y-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-xs font-mono text-slate-600 dark:text-slate-400">
            <Link
              href="/library"
              className="flex items-center gap-1 text-slate-600 hover:text-cyan-700 dark:text-slate-400 dark:hover:text-cyan-300 transition-colors"
            >
            ...
            <span>/</span>
            <span className="text-cyan-700 dark:text-cyan-300 font-semibold">{algorithm.name}</span>
          </div>

          {/* Primary Try in Prism Button */}
          <button
            onClick={handleTryInPrism}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 dark:bg-cyan-500 dark:hover:bg-cyan-400 text-white dark:text-slate-950 font-bold text-sm transition-all shadow-md shadow-cyan-600/20 cursor-pointer active:scale-95"
          >
        ...
        {/* Hero Banner */}
        <section className="bg-gradient-to-br from-white via-slate-50 to-cyan-50/40 dark:from-slate-900 dark:via-slate-900/90 dark:to-cyan-950/30 border border-slate-300 dark:border-slate-800 rounded-2xl p-6 md:p-8 shadow-xs dark:shadow-xl space-y-4">
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-slate-100 border border-slate-300 text-slate-700 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 text-xs font-mono">
          ...
          <p className="text-sm md:text-base text-slate-700 dark:text-slate-300 max-w-3xl leading-relaxed">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2">
            {algorithm.tags.map((tag) => (
              <span
                key={tag}
                className="text-xs font-mono px-2.5 py-0.5 rounded-md bg-slate-100 text-slate-800 border border-slate-300 dark:bg-slate-800/80 dark:text-slate-300 dark:border-slate-700/60 shadow-2xs"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-6 items-start">
          ...
          <div className="space-y-6">
            {/* What This Does */}
            <section className="bg-white dark:bg-slate-900/70 border border-slate-300 dark:border-slate-800 rounded-xl p-6 shadow-xs dark:shadow-md space-y-3">
              <div className="flex items-center gap-2 text-cyan-700 dark:text-cyan-400 font-mono text-xs uppercase tracking-wider font-semibold">
            ...
            <section className="bg-white dark:bg-slate-900/70 border border-slate-300 dark:border-slate-800 rounded-xl p-6 shadow-xs dark:shadow-md space-y-3">
              ...
              <ol className="list-none space-y-2.5 text-sm text-slate-700 dark:text-slate-300 p-0 m-0">
                {algorithm.howItWorks.map((step, idx) => (
                  <li key={idx} className="flex items-start gap-3">
                    <span className="flex items-center justify-center w-5 h-5 rounded-full bg-cyan-100 border border-cyan-300 text-cyan-800 dark:bg-cyan-950 dark:border-cyan-500/40 dark:text-cyan-300 text-xs font-mono font-bold shrink-0 mt-0.5">
                      {idx + 1}
                    </span>
            ...
            {/* Complexity Analysis */}
            <section className="bg-white dark:bg-slate-900/70 border border-slate-300 dark:border-slate-800 rounded-xl p-6 shadow-xs dark:shadow-md space-y-4">
              <div className="flex items-center gap-2 text-cyan-700 dark:text-cyan-400 font-mono text-xs uppercase tracking-wider font-semibold">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {/* Time Complexity */}
                <div className="bg-slate-50 dark:bg-slate-950/80 border border-slate-300 dark:border-slate-800 p-3.5 rounded-lg space-y-2">
                  <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-400 font-mono font-semibold">
                    <Clock className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400" />
                    <span>Time Complexity</span>
                  </div>
                  <div className="text-xs space-y-1 font-mono">
                    <div className="text-slate-700 dark:text-slate-300">Best: <span className="text-emerald-700 dark:text-emerald-400 font-bold">{algorithm.timeComplexity.best}</span></div>
                    <div className="text-slate-700 dark:text-slate-300">Average: <span className="text-amber-700 dark:text-amber-400 font-bold">{algorithm.timeComplexity.average}</span></div>
                    <div className="text-slate-700 dark:text-slate-300">Worst: <span className="text-rose-700 dark:text-rose-400 font-bold">{algorithm.timeComplexity.worst}</span></div>
                  </div>
                  {algorithm.timeComplexity.explanation && (
                    <p className="text-[11px] text-slate-600 dark:text-slate-400 pt-1 border-t border-slate-200 dark:border-slate-800/80 leading-relaxed">
                      {algorithm.timeComplexity.explanation}
                    </p>
                  )}
                </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-950/80 border border-slate-300 dark:border-slate-800 p-3.5 rounded-lg space-y-2">
                  ...
                  <div className="text-xs space-y-1 font-mono">
                    <div className="text-slate-700 dark:text-slate-300">Worst Case: <span className="text-purple-700 dark:text-purple-300 font-bold">{algorithm.spaceComplexity.worst}</span></div>
                  </div>
            ...
            {algorithm.prerequisites.length > 0 && (
              <section className="bg-white dark:bg-slate-900/70 border border-slate-300 dark:border-slate-800 rounded-xl p-5 shadow-xs dark:shadow-md space-y-2.5">
                ...
                <div className="flex flex-wrap gap-2">
                  {algorithm.prerequisites.map((prereq, idx) => (
                    <span
                      key={idx}
                      className="px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-xs text-slate-800 dark:text-slate-300 font-medium"
                    >
                      ✓ {prereq}
                    </span>
                  ))}
            ...
            <section className="bg-gradient-to-br from-purple-50 to-white dark:from-purple-950/20 dark:to-slate-900/70 border border-purple-200 dark:border-purple-500/20 rounded-xl p-6 shadow-xs dark:shadow-md space-y-3">
              <div className="flex items-center gap-2 text-purple-700 dark:text-purple-400 font-mono text-xs uppercase tracking-wider font-semibold">
              ...
              <div className="space-y-2">
                {algorithm.suggestedTutorQuestions.map((q, idx) => (
                  <div
                    key={idx}
                    className="p-2.5 rounded-lg bg-white dark:bg-slate-950/80 border border-purple-200 dark:border-slate-800 text-xs text-slate-800 dark:text-slate-200 font-mono flex items-start gap-2 shadow-2xs"
                  >
