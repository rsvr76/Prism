...
export default function ExecutionOutputTab() {
  ...
  return (
    <div
      ...
      className={`absolute top-4 left-1/2 -translate-x-1/2 z-30 flex flex-col pointer-events-auto bg-white/95 dark:bg-[#070b16]/95 backdrop-blur-md border border-slate-300 dark:border-slate-800 rounded-xl shadow-2xl overflow-hidden transition-all duration-200 animate-in fade-in zoom-in-95 ${
        isCompact ? "w-[300px] sm:w-[340px]" : "w-[90vw] sm:w-[420px] md:w-[480px]"
      }`}
      style={{ maxHeight: "calc(100% - 32px)" }}
    >
      {/* Output Header */}
      <div className="h-9 px-3 bg-slate-100/90 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between shrink-0 select-none">
        ...
        <button
          onClick={closeOutput}
          className="p-1 rounded-md text-slate-500 hover:text-slate-800 hover:bg-slate-200/60 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-800/60 transition-colors cursor-pointer"
      ...
      <div className="p-3 overflow-y-auto font-mono text-xs text-slate-800 dark:text-slate-200 space-y-1 custom-scrollbar max-h-72 bg-slate-50/50 dark:bg-transparent">
        {isRunning ? (
          ...
          stdout.map((line, idx) => (
            <div key={idx} className="flex gap-2 text-emerald-800 dark:text-emerald-300 leading-relaxed break-words font-medium">
