    ...
    <div className="h-full flex flex-col justify-between overflow-y-auto p-4 space-y-4 text-slate-200">
      ...
      {!isExplaining && currentExplanation && (
        <div className="flex-1 space-y-3.5 min-h-0">
          ...
          <div className="rounded-lg bg-emerald-950/20 border border-emerald-500/30 overflow-hidden shadow-xs">
            <div className="px-3 py-1.5 bg-emerald-950/40 border-b border-emerald-500/30 flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-emerald-300 text-xs font-mono font-bold uppercase tracking-wide">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                <span>OBSERVED IN EXECUTION</span>
              </div>
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-950 border border-emerald-500/40 text-emerald-300 font-mono font-semibold">
                Trace Truth
              </span>
            </div>
            ...
            <div className="p-3 space-y-2">
              <div className="text-xs text-slate-300 font-mono">
                <span className="text-slate-400">Step {currentStep}: </span>
                <span className="text-white font-semibold">Executed Line {frame.line}</span>
              </div>

              {currentExplanation.changes.length > 0 ? (
                <ul className="space-y-1 text-xs text-slate-200 font-mono bg-slate-950/60 p-2.5 rounded-md border border-slate-800/80">
                ...
                <p className="text-xs text-slate-400 italic">No variables mutated at this execution step.</p>
              )}
            </div>
          </div>

          {/* SECTION 2: EXPLAINED (Pedagogical Insight) */}
          <div className="rounded-lg bg-purple-950/15 border border-purple-500/30 overflow-hidden shadow-xs space-y-0">
            <div className="px-3 py-1.5 bg-purple-950/40 border-b border-purple-500/30 flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-purple-300 text-xs font-mono font-bold uppercase tracking-wide">
                <Sparkles className="w-3.5 h-3.5 text-purple-400" />
                <span>EXPLAINED BY PRISM AI</span>
              </div>
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-purple-950 border border-purple-500/40 text-purple-300 font-mono font-semibold">
                Pedagogy
              </span>
            </div>
            ...
            <div className="p-3 space-y-3">
              ...
              {currentExplanation.dataStructureInsight && (
                <div className="p-2.5 rounded-md bg-purple-950/40 border border-purple-800/50 space-y-1">
                  <div className="flex items-center gap-1.5 text-purple-300 text-[11px] font-mono font-bold">
              ...
              {/* Key Learning Point */}
              <div className="p-2.5 rounded-md bg-slate-900/80 border border-slate-800 space-y-1">
                <div className="flex items-center gap-1.5 text-cyan-300 text-[11px] font-mono font-bold">
