/**
 * Server-Side Next.js Route Handler: POST /api/ai/explain-step
 * Securely calls configured LLM with Zod validation.
 */

import { NextResponse } from "next/server";
import { ExplainStepRequestSchema } from "@/lib/ai/schemas";
import { generateStepExplanation } from "@/lib/ai/llmClient";

export async function POST(req: Request) {
  try {
    let body: unknown;
    try {
      body = await req.json();
    } catch {
      return NextResponse.json(
        { success: false, error: "Malformed JSON in request body." },
        { status: 400 }
      );
    }

    // 1. Zod validate incoming request
    const parseResult = ExplainStepRequestSchema.safeParse(body);
    if (!parseResult.success) {
      const errorMsg = parseResult.error.issues.map((e) => `${e.path.join(".")}: ${e.message}`).join(", ");
      return NextResponse.json(
        { success: false, error: `Invalid request payload: ${errorMsg}` },
        { status: 400 }
      );
    }

    const { context, sourceCode } = parseResult.data;

    // 2. Call LLM provider with grounding context
    const explanation = await generateStepExplanation({
      context,
      sourceCode,
    });

    return NextResponse.json({
      success: true,
      data: explanation,
    });
  } catch (error: any) {
    console.error("Prism Step Explainer API Error:", error);
    return NextResponse.json(
      {
        success: false,
        error: error.message || "An unexpected error occurred while generating the explanation.",
      },
      { status: 500 }
    );
  }
}
